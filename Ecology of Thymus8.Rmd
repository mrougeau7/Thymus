...
Since the Bray CUrtis Dissimilarity Index failed to detect any compositional differences between the wild-type and mutant thymusus, our next step was to determine if there were any organizational differences.  (This will also be used to determine if the organization of WT thymuses are similar).  We used Connected Component Labeling (cite?) to group connected components (clusters) together.  We were then able to calculate the centrod of each cluster and calculate the distance between clusters of different regions.
