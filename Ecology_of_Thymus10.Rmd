...
mat<-matrix(data=NA,nrow=22500,ncol=33) #this will change with size of matrix and # of sections/cell types
...
for(i in toUse0){
  ...
  obj <- my.xy$counts
  obj[1,1] = obj[1,1]-1
  obj[150,1] = obj[150,1]-1
  obj[1,150] = obj[1,150]-1
  obj[150,150]=obj[150,150]-1
  count<- as.vector(obj)
  toFill[,i]<-count    
...
#mat <-read.table("Totalcounts150_Thymus1_2.txt", head=T, sep="\t")
...
#spe.kmeans_WT1_All <- kmeans(result0, centers=cluster, nstart=1000)
#spe.kmeans_WT1_S1 <- kmeans(result1, centers=cluster, nstart=1000)
#spe.kmeans_WT1_S2 <- kmeans(result2, centers=cluster, nstart=1000)
#spe.kmeans_WT1_S3 <- kmeans(result3, centers=cluster, nstart=1000)
...
for (i in 1:max(spe.kmeans_WT1_All$cluster)){ 
  barplot(colSums(result0[which(spe.kmeans_WT1_All$cluster==i),]),main=i,ylim=c(0,11500)) 
  # we pick out desired cluster and plot
}

my.order<-c(1,2,3,4) # define the order we want to plot panels
par(mfrow=c(2,2)) # make 4 subplots in 2x2 style
for (i in 1:max(spe.kmeans_WT1_S1$cluster)){ 
  barplot(colSums(result1[which(spe.kmeans_WT1_S1$cluster==i),]),main=i,ylim=c(0,11500)) 
  # we pick out desired cluster and plot
}

my.order<-c(1,2,3,4) # define the order we want to plot panels
par(mfrow=c(2,2)) # make 4 subplots in 2x2 style
for (i in 1:max(spe.kmeans_WT1_S2$cluster)){ 
  barplot(colSums(result2[which(spe.kmeans_WT1_S2$cluster==i),]),main=i,ylim=c(0,11500)) 
  # we pick out desired cluster and plot
}

my.order<-c(1,2,3,4) # define the order we want to plot panels
par(mfrow=c(2,2)) # make 4 subplots in 2x2 style
for (i in 1:max(spe.kmeans_WT1_S3$cluster)){ 
  barplot(colSums(result3[which(spe.kmeans_WT1_S3$cluster==i),]),main=i,ylim=c(0,11500)) 
  # we pick out desired cluster and plot
}
...
b1<-colSums(result0[which(spe.kmeans_WT1_All$cluster==1),])
b2<-colSums(result0[which(spe.kmeans_WT1_All$cluster==2),])
b3<-colSums(result0[which(spe.kmeans_WT1_All$cluster==3),])
b4<-colSums(result0[which(spe.kmeans_WT1_All$cluster==4),])

b1_S1<-colSums(result1[which(spe.kmeans_WT1_S1$cluster==1),])
b2_S1<-colSums(result1[which(spe.kmeans_WT1_S1$cluster==2),])
b3_S1<-colSums(result1[which(spe.kmeans_WT1_S1$cluster==3),])
b4_S1<-colSums(result1[which(spe.kmeans_WT1_S1$cluster==4),])

b1_S2<-colSums(result2[which(spe.kmeans_WT1_S2$cluster==1),])
b2_S2<-colSums(result2[which(spe.kmeans_WT1_S2$cluster==2),])
b3_S2<-colSums(result2[which(spe.kmeans_WT1_S2$cluster==3),])
b4_S2<-colSums(result2[which(spe.kmeans_WT1_S2$cluster==4),])

b1_S3<-colSums(result3[which(spe.kmeans_WT1_S3$cluster==1),])
b2_S3<-colSums(result3[which(spe.kmeans_WT1_S3$cluster==2),])
b3_S3<-colSums(result3[which(spe.kmeans_WT1_S3$cluster==3),])
b4_S3<-colSums(result3[which(spe.kmeans_WT1_S3$cluster==4),])
...
#The way to read the last plots is for something like "2-3", positive bars mean that cell type is more common in cluster 2 (than 3); #negative bars mean the opposite. The higher the absolute number on the y-axis, the greater the difference is in that cell count between #the two clusters. I think (as John suggested), that we can move ahead and be more formal with this analysis (e.g. Bray Curtis dissimilar#ity), but this is a useful first step.
```
...
df<-result0[which(spe.kmeans_WT1_All$cluster==1),] %>% colSums() %>% t()   # example of chaining. 
for (j in 2:4){  
  tmp<-result0[which(spe.kmeans_WT1_All$cluster==j),] %>% colSums() %>% t() 
  df<-rbind(df,tmp)
}

df_S1<-result1[which(spe.kmeans_WT1_S1$cluster==1),] %>% colSums() %>% t()
for (j in 2:4){  
  tmp_S1<-result1[which(spe.kmeans_WT1_S1$cluster==j),] %>% colSums() %>% t() 
  df_S1<-rbind(df_S1,tmp_S1)
}

df_S2<-result2[which(spe.kmeans_WT1_S2$cluster==1),] %>% colSums() %>% t()
for (j in 2:4){  
  tmp_S2<-result2[which(spe.kmeans_WT1_S2$cluster==j),] %>% colSums() %>% t() 
  df_S2<-rbind(df_S2,tmp_S2)
}

df_S3<-result3[which(spe.kmeans_WT1_S3$cluster==1),] %>% colSums() %>% t()
for (j in 2:4){  
  tmp_S3<-result3[which(spe.kmeans_WT1_S3$cluster==j),] %>% colSums() %>% t() 
...
#dfTOTAL_R<-rbind(df_S1_WT1_R,df_S2_WT1_R, df_S3_WT1_R, df_S1_WT2_R, df_S2_WT2_R, df_S3_WT2_R, df_S1_WT1_R,
#df_S2_WT1_R, df_S3_WT1_R, df_S1_Mutant2_R, df_S2_Mutant2_R, df_S3_Mutant2_R)
...
7. jackknifing : removing one cell type at a time
...
7. Cataldo S, Ficarra E, Macii E (1509) Automated Discrimination of Pathological Regions in Tissue Images:
...
(10. Magurran AE (1504) Measuring Biological Diversity. Malden, MA: Blackwell Publishing.)
...
http://www.scielo.br/scielo.php?pid=S1516-89131509000600013&script=sci_arttext
...
assume we have vector (e.g. 15000 pixels, numbered)
