...
Our approach is designed to take advantage of the cellular complexity of the thymus to provide a sensitive and quantitative method to measure changes over time. We use immunostaining on sagittal serial sections of a wildtype mouse thymus in order to identify distinct cellular subsets, followed by the use of novel computational approaches in order to quantitatively identify known and cryptic cellular spatial relationships. These methods are designed to replace the largely descriptive histological and IHC-based analysis currently available in the field.  First, we used the K means clustering algorithm to identify geographical clusters of cell types based on compositional similarities, position, and abundance. This approach has previously been successful in delineating distinct cell sub-population in tissue samples (cite) {INCLUDE MORE INFO}.K means clustering allows us to test the structure of thymus in a statistical framework that can provide unambiguous support for the number of compositionally distinct thymic regions. We then used a range of K values to compare to previously identified regions of the thymus and to identify potentially cryptic organization. This clustering algorithm generates a posterior probability that any subsample (quadrat) belongs to each of the k different groups, while maximizing the ratio of compositional differences between groups to compositional variation within groups.  These k groups are presumable functionally distinct, and a key hypothesis we will test is whether Kmutant is significantly less than kwildtype for each comparison. 

13. Veronika M, Evans J, Matsudaira P, Welsch R, Rajapakse J (2009) Sub-population analysis based on
...
In ecology, the identity and relative abundance of species can be compared across communities using various dissimilarity indicies (10. Magurran AE (2004) Measuring Biological Diversity. Malden, MA: Blackwell Publishing.) The Bray Curtis Dissimilarity Index in particular is a well characterized statistical method. We applied Bray Curtis to our samples to determine the degree of similarity between clusters produced from K means clustering [CITE]. Bray Curtis was also used to make comparisons between different thymic samples and subsamples, and can also be compared to ecosystems.

connected component labeling

We then take this approach one step further by applying these same approaches described above to a mutant mouse thymus in order to determine if there are any detectable organizational differences compared to the wildtype. We selected the Aire knockout Mouse (CITE) for several reasons.  First, the Aire knockout is well characterized: DESCRIBE PHENOTYPE HERE?? there is debate as to whether mutant mice double in mTEC number (particularly with respect to K14).  There is also debate as to whether dendritic cell organization and number is altered; there are differing reports in the literature [CITE]. From an ecological standpoint, we are interested in determining the degree to which Aire plays. Although we know Aire is functionally important, does it act as a Keystone species? Will the removal of Aire cause a shift BLAH. Or will there be no discernable organizational differences? 
