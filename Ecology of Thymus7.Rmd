...
my.mat_M1 = v0.1_Mutant1
my.mat_M1_S1 = v1.1_Mutant1
my.mat_M1_S2 = v2.1_Mutant1
my.mat_M1_S3 = v3.1_Mutant1

my.mat_M2 = v0.1_Mutant2
my.mat_M2_S1 = v1.1_Mutant2
my.mat_M2_S2 = v2.1_Mutant2
my.mat_M2_S3 = v3.1_Mutant2

my.mat_WT1 = v0.1_WT1
my.mat_WT1_S1 = v1.1_WT1
my.mat_WT1_S2 = v2.1_WT1
my.mat_WT1_S3 = v3.1_WT1
...
We used the K means clustering algorithm to identify geographical clusters of cell types based on compositional similarities and abundance. We used a range of K values to compare to previously identified regions of the thymus and to identify cryptic organization. K=3 reproduced the cortical and medullary regions, which was expected since a majority of the antibody markers correspond to cell types specifically located within these regions. We increased K to see if we could reproduce the CMJ or subcapsular region based on compositional similarities, since these areas lack region specific antibodies.  K=4 results in the subdivision of the medulla, which could either represent the CMJ or a new unidentified functional region.  Increasing K further results in clusters forming throughout the medulla and eventually throughout the cortex. At higher K values, we see formation of the subcapsular region.  We focused on K=4 because we were interested in the formation of 2 clusters (subdivision) in the medullary region.
TALK ABOUT NORMALIZED??

Next, we produced graphs to show us how clusters were forming as well as to help us determine what cell types were driving the clustering.  The first graph showed the amount of cells per cell type in each cluster. The second compared the proportions of cells across clusters. Compared to mutant.    
...
How do the mutant and WT differ spatially? Can we prove they are different statistically? Want to quantify extent to which mutants are significantly less structured than wild-type. connected component labeling-distributional interactions
...
```{r}
ccl.mat2<-ifelse(my.mat==2,1,0)
ccl.mat3<-ccl.mat2*ccl.mat
image(ccl.mat3)
length(unique(as.vector(ccl.mat3))-1)
image(ccl.mat3[20:30,70:80])


my.mat2<-ccl.mat2*my.mat
new.ccl<-ConnCompLabel(ccl.mat2)
image(new.ccl,col=rev(gray.colors(30)))
new.ccl2<-ifelse(new.ccl>0,1,0)
image(new.ccl2)
length(unique(as.vector(new.ccl))-1)
```